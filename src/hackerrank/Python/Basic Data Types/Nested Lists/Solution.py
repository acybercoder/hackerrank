...
subsequent lines describe each student over 
...
if __name__ == '__main__':
    for _ in range(int(input())):
        name = input()
        score = float(input())
        transcript.append([name, score])
    second_lowest = sorted(set([score for _, score in transcript]))[1]
    print(os.linesep.join([name for name, score in sorted(transcript) if score==second_lowest]))
